...
class RFQCritic(RLNetwork):
    def __init__(self, sa_dim=4, embedding_dim=4, rf_num=256, sigma=0.0, learn_rf=False, **kwargs):
        super().__init__()
        self.n_layers = 1
        ...
        if embedding_dim != -1:
            ...
            embedding_dim = sa_dim
            self.embed = nn.Linear(sa_dim, sa_dim)
...
class RFSACAgent(SACAgent):
    ...
    def pendubot_f_6d(self, states, action):
        ...
        acc = torch.reciprocal(m11 * m22 - m21 ** 2 + 1e-8) * mass_inv @ (action -
                                                                          torch.matmul(c_matrix, states[:, -2:][:, :,
                                                                                                 np.newaxis]) - g_vec)
    ...
    def get_reward(self, obs, action):
        if self.dynamics_type == 'Pendulum':
            ...
            if self.sin_input is False:
                ...
                reward = -(torch.sum(torch.multiply(torch.tensor([0.01, 0.001, 0.001], device=device), obs_no_theta),
                                     dim=1)
...
class DensityConstrainedLagrangianAgent(RFSACAgent):
    ...
    def __init__(self,
        ...
        self.multiplier = util.mlp(input_dim=state_dim,
                                   hidden_dim=hidden_dim,
                                   output_dim=1,
                                   hidden_depth=2,
                                   output_mod=nn.Softplus())
        self.multiplier_optimizer = torch.optim.Adam(self.multiplier.parameters(),
                                                     lr,
                                                     betas=[0.9, 0.999])

        self.density = DensityCritic(s_dim=state_dim, sigma=sigma, rf_num=rf_num, learn_rf=learn_rf, **kwargs).to(
            device)
    ...
    def density_critic_step(self, batch, alg='td'):
        ...
        if alg == 'td':
            ...
            with torch.no_grad():
                ...
                initial_density_state = self.get_initial_density(state)
