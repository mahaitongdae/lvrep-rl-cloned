...
def Prepare_Region_LQR(env_name, Nstate,NKoopman, thdot_weight=0.1, u_weight = 0.0):
	x_ref = np.array([0.0, 0.0, 0.5, 0.0, 1.0, 0.0, 0.0])
...
def Cost(observations,u_list,Q,R,x_ref,gamma = 0.99):
	...
	for s in range(steps):
		...
		x_cost = observations[0,s] ** 2 + (observations[2,s] - 0.5) ** 2
		# loss += x_cost * gamma**s
		loss += x_cost
	return loss
...
def main(tr_seed = 0, save_traj = False, samples = 80000, sigma = 0.0, euler = False):
	method = "KoopmanU"
	suffix = "Quadrotor2D-v2"
	env_name = "Quadrotor2D-v2"
	...
	phi_cost = _phi[0]**2 + (_phi[2] - 0.5)**2  #should be (1,1) shape
	...
	simulator.set_param(t_step = 0.008)
	simulator.setup()
