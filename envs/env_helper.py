...
def env_creator_pendulum(env_config):
    ...
    env = gymnasium.make('Pendulum-v2', sigma=env_config.get('noise_scale'))
    ...
    # else:
    #     env = TransformReward(env, lambda r: env_config.get('reward_scale') * r)
    # if env_config.get('noise_scale') > 0.0:
    #     env = NoisyObservationWrapper(env, noise_scale=env_config.get('noise_scale', 1), noise_add_dim=[1])
    return env
