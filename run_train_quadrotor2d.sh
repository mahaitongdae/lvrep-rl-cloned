...
#        for NYSTROM_SAMPLE_DIM in $RF_NUM; currently not using top K
        for SEED in 0; do # --critic_lr 1e-3
          python main.py --use_nystrom --reward_exponential --critic_lr 1e-3 --alg $ALG --env Quadrotor2D-v2 --sigma $SIGMA --max_timesteps $MAX_TIMESTEPS --rf_num $RF_NUM --nystrom_sample_dim $RF_NUM --seed $SEED
        done
#        done
      done
    done
