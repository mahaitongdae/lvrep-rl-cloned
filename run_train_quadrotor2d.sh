...
#        for NYSTROM_SAMPLE_DIM in 8192; do # currently not using top K
        for SEED in 2 3 4; do # --critic_lr 1e-3
...
#          python main.py --use_random_feature --reward_exponential --learn_rf --critic_lr 1e-3 --alg $ALG --env Quadrotor2D-v2 --sigma $SIGMA --max_timesteps $MAX_TIMESTEPS --rf_num $RF_NUM --seed $SEED
          for R in 1.0; do
            python main.py --use_random_feature --robust_radius $R --device cuda --reward_exponential --learn_rf --critic_lr 1e-3 --alg $ALG --env Quadrotor2D-v2 --sigma $SIGMA --max_timesteps $MAX_TIMESTEPS --rf_num $RF_NUM --seed $SEED
          done
        done
      done
    done
